...
/**
 * 路由表单例类，读取路由配置，获取路由表中的服务地址
 *
 * @author zhangbotao
 */
public class RouteTableSingleton {

    private RouteTableSingleton() {
    }
    ...
    public String getTargetUrl(String url) {
        for (Map<String, String> table : route) {
            ...
            if (index == 0) {
                ...
                // eg：http://192.168.101.104:8080/group1
                return target + url;
            }
        }
        return null;
    ...
    public void readJsonConfigByGson() {
    ...
    public void readJsonConfig() {
        try (InputStream inputStream = this.getClass().getClassLoader().getResourceAsStream("route.json")) {
            Map<String, Object> config = JSON.parseObject(inputStream, new TypeToken<Map<String, Object>>() {
            }.getType());
